...
\begin{document}
	...
	%================================
	\clip (A) rectangle (C);
	\def\maunen{orange!80!white} %màu nền cả trang
	\def\mautren{orange!60!purple} %màu trên
	\def\mauchu{green!80!black} %màu chữ
	%======================== PHỦ CẢ TRANG
...
%	\draw ([yshift=-5mm]KN) node[green,below]{\fontfamily{qag}\fontsize{8pt}{1pt}\selectfont\bfseries VỚI CUỘC SỐNG};
	...
	\draw[purple] (TT) circle (10cm);
	\draw[purple] ([xshift=0.5cm]TT) circle (10cm);
	\draw[purple] ([xshift=0.25cm,yshift=-0.5cm]TT) circle (10cm);
	\draw[purple] ([xshift=0.25cm,yshift=0.5cm]TT) circle (10cm);
	\draw[purple] ([xshift=-0.25cm,yshift=-0.5cm]TT) circle (10cm);
	\draw[purple] ([xshift=-0.5cm,yshift=0.5cm]TT) circle (10cm);
	\fill[purple!70] ([xshift=3mm,yshift=-1mm]TT) circle (9cm);
...
%	\node[text=teal,xscale=0.7,yscale=0.7,left] at ([yshift=-3.35cm,xshift=-6mm]GG) {\fontfamily{qag}\fontsize{20}{0}\selectfont\bfseries (Chân trời sáng tạo)};
	...
	%--Hình 
	\clip (TT) circle (7.5cm) node {\includegraphics[width=13.5cm]{nen11-sem2}};
	%=============================
\end{tikzpicture}
\begin{tikzpicture}[overlay,remember picture,transform shape,>=stealth,line join=round,line cap=round,font=\footnotesize]
	%================================
	...
	%================================
	\clip (A) rectangle (C);
	\def\maunen{orange!80!purple} %màu nền cả trang
	\def\mautren{orange!60!purple} %màu trên
	\def\mauchu{green!70!black} %màu chữ
...
%	
	...
	%============================= 6
	\coordinate (TAM) at ([xshift=2.8cm,yshift=-0.65cm]GG);
	\fill[\maunen] ([xshift=3mm]TAM) circle (2.85cm);	
	\fill[\mautren,draw=white, line width=2pt] (TAM) circle (2.75cm);
	\node[text=white,xscale=6.5,yscale=7.2] at (TAM) {\fontfamily{qag}\fontsize{18}{0}\selectfont\bfseries 11}; % LỚP
	\node[text=blue,scale=1.25] at ([yshift=-3.75cm]TAM) {\fontfamily{put}\fontsize{20}{0}\selectfont\bfseries HỌC KÌ 2};	
	\node[text=\mauchu,xscale=1.05,yscale=1.05,left] at ([yshift=-2.45cm,xshift=-12mm]GG) {\fontfamily{qag}\fontsize{20}{0}\selectfont\bfseries THEO CHƯƠNG TRÌNH MỚI};
		%=============================NXB
