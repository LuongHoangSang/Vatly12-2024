...
\begin{document}
	...
	%================================
	\clip (A) rectangle (C);
	\def\maunen{teal!30!cyan} %màu nền cả trang
	\def\mautren{blue!50!black} %màu trên
...
%	\draw ([yshift=-5mm]KN) node[green,below]{\fontfamily{qag}\fontsize{8pt}{1pt}\selectfont\bfseries VỚI CUỘC SỐNG};
	...
	\draw[blue] (TT) circle (10cm);
	\draw[blue] ([xshift=0.5cm]TT) circle (10cm);
	\draw[blue] ([xshift=0.25cm,yshift=-0.5cm]TT) circle (10cm);
	\draw[blue] ([xshift=0.25cm,yshift=0.5cm]TT) circle (10cm);
	\draw[blue] ([xshift=-0.25cm,yshift=-0.5cm]TT) circle (10cm);
	\draw[blue] ([xshift=-0.5cm,yshift=0.5cm]TT) circle (10cm);
	\fill[blue!70] ([xshift=3mm,yshift=-1mm]TT) circle (9cm);
...
%	\node[text=teal,xscale=0.7,yscale=0.7,left] at ([yshift=-3.35cm,xshift=-6mm]GG) {\fontfamily{qag}\fontsize{20}{0}\selectfont\bfseries (Chân trời sáng tạo)};
	...
	%--Hình 
	\clip (TT) circle (7cm) node {\includegraphics[width=14cm]{nen10}};
	%=============================
\end{tikzpicture}
\begin{tikzpicture}[overlay,remember picture,transform shape,>=stealth,line join=round,line cap=round,font=\footnotesize]
	%================================
	...
	%================================
	\clip (A) rectangle (C);
	\def\maunen{teal!80!white} %màu nền cả trang
	\def\mautren{blue!80!black} %màu trên
	\def\mauchu{teal} %màu chữ
...
%	}
	...
	%============================= 6
	\coordinate (TAM) at ([xshift=2.8cm,yshift=-0.65cm]GG);
	\fill[blue!70!white] ([xshift=3mm]TAM) circle (2.85cm);	
	...
	\node[text=red,xscale=1,yscale=1] at (NXB) {\fontfamily{qag}\fontsize{15}{0}\selectfont\bfseries LỚP VẬT LÝ CÔ THẢO - THẦY SANG};
	\node[text=red,xscale=1,yscale=1] at (TL) {\fontfamily{qag}\fontsize{15}{0}\selectfont\itshape (Tài liệu lưu hành nội bộ)};
\end{tikzpicture}
\end{document}
