...
\begin{document}
	...
	%================================
	\clip (A) rectangle (C);
	\def\maunen{teal!60!white} %màu nền cả trang
	\def\mautren{teal!80!black} %màu trên
...
%	\node[text=teal,xscale=0.7,yscale=0.7,left] at ([yshift=-3.35cm,xshift=-6mm]GG) {\fontfamily{qag}\fontsize{20}{0}\selectfont\bfseries (Chân trời sáng tạo)};
	...
	%--Hình 
	\clip (TT) circle (7.5cm) node {\includegraphics[width=11cm]{nen10-sem2}};
	%=============================
\end{tikzpicture}
\begin{tikzpicture}[overlay,remember picture,transform shape,>=stealth,line join=round,line cap=round,font=\footnotesize]
	%================================
...
%	
	...
	\def\mauchu{red!90!black} %màu chữ
	\def\maubong{black}
	\foreach \i in {0,0.01,...,\n}{
		\pgfmathsetmacro\rr{\n-\i}
	...
	%============================= 6
	\coordinate (TAM) at ([xshift=2.8cm,yshift=-0.65cm]GG);
	\fill[purple!40!white] ([xshift=3mm]TAM) circle (2.85cm);	
	\fill[\mautren,draw=white, line width=2pt] (TAM) circle (2.75cm);
	\node[text=white,xscale=6.5,yscale=7.2] at (TAM) {\fontfamily{qag}\fontsize{18}{0}\selectfont\bfseries 10};
	\node[text=blue,scale=1.25] at ([yshift=-3.75cm]TAM) {\fontfamily{put}\fontsize{20}{0}\selectfont\bfseries HỌC KÌ 2};	
	\node[text=\mauchu,xscale=1.05,yscale=1.05,left] at ([yshift=-2.45cm,xshift=-2.5cm]GG) {\fontfamily{qag}\fontsize{20}{0}\selectfont\bfseries\centering \textit{(Tài liệu học sinh)}};
		%=============================NXB
